...
uses
  ...
  FireDAC.Comp.Client, IniFiles, Controls, Vcl.Dialogs,
  Winapi.Windows;
...
begin
  try
    ...
    MySQL_DriverLink.VendorLib:= ArquivoINI.ReadString('CONFIG', 'DLL_Path', sEXEPath + '\libmysql.dll');
    ...
    frmPrincipal.pnPedido.Enabled:= True;
    frmPrincipal.pnItens.Enabled:= True;
    frmPrincipal.dbgItens.Enabled:= True;
  except
    on E: Exception do
    begin
       ShowMessage('Erro ao se conectar ao banco de dados: ' + #13 + E.Message);

       frmPrincipal.pnPedido.Enabled:= False;
       frmPrincipal.pnItens.Enabled:= False;
       frmPrincipal.dbgItens.Enabled:= False;

       frmPrincipal.bConfig.Click;
    end;
  end;
end;
...
begin
  frmPrincipal.bExcluirPedido.Visible:= (qPedidos.FieldByName('CODCLI').AsInteger <= 0);
